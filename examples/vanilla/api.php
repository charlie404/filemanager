...
/**
 * Single-file backend for the @charlie404/filemanager Vanilla example.
 *
 * It implements the full REST contract the `<file-manager>` element expects,
 * over a plain filesystem — no database. Metadata lives in a per-folder sidecar
 * `.meta.json`. This is a self-contained adaptation of the library's reference
 * backend (server/router.php).
 *
 * Run it as PHP's built-in router so the same process serves the API, the
 * uploaded files, this example's page and the built library bundle:
 *
 *   php -S localhost:8000 api.php
 *
 * Then open http://localhost:8000/ — the page and the API are same-origin, so
 * there is no proxy and no CORS to configure.
 *
 * Routes:
 *   GET  /                      -> index.html (this example's page)
 *   GET  /index.html            -> index.html
 *   GET  /dist/index.js         -> the built library bundle (../../dist/index.js)
 *   GET  /uploads/...           -> a stored file
 *   /api/...                    -> the REST contract below
 *
 * API contract (base path /api):
 *   GET    /folders
 *   POST   /folders            {name, parent}
 *   PUT    /folders/{id}       {name}
 *   DELETE /folders/{id}
 *   GET    /files?folder=ID
 *   POST   /files              (multipart: file, folder)
 *   PUT    /files/{id}         {name?, meta?}
...
/** Repo root so we can reach the built bundle at ../../dist/index.js. */
...
// Same-origin: no CORS headers required. We keep an OPTIONS short-circuit only
// so preflights (should any tooling send them) never fall through to a 404.
if ($method === 'OPTIONS') {
    http_response_code(204);
    exit;
}

// --- serve this example's page ---------------------------------------------
if ($uri === '/' || $uri === '/index.html') {
    serveStatic(__DIR__ . '/index.html', 'text/html; charset=utf-8');
}

// --- serve the built library bundle (so the page needs no build step) -------
if ($uri === '/dist/index.js') {
    $bundle = DIST . '/index.js';
    if (!is_file($bundle)) {
        http_response_code(404);
        header('Content-Type: text/plain; charset=utf-8');
        echo "Built bundle not found at $bundle.\n";
        echo "Run `npm run build` in the repo root first, or copy dist/index.js next to this file.\n";
        exit;
    }
    serveStatic($bundle, 'text/javascript; charset=utf-8');
}
...
function cropFile(array $d): array
{
    if (!function_exists('imagecreatetruecolor')) {
        throw new RuntimeException('The PHP GD extension is required for cropping');
    }
 ...
 *
 * Existing targets are compared via realpath (so symlinks can't smuggle a path
 * out of ROOT). Not-yet-existing targets (e.g. a dedupe/upload destination) are
 * compared lexically — crucially against the *lexically* normalised ROOT, so a
 * symlinked storage prefix (macOS resolves /tmp -> /private/tmp) never produces
 * a false negative for a legitimate in-bounds path.
 */
function safePath(string $id): ?string
{
    ...
    if ($real !== false) {
        ...
        return str_starts_with($real, (string) $rootReal . '/') || $real === $rootReal ? $abs : null;
    }
    // Target does not exist yet — validate the collapsed lexical path.
    $real = lexicalResolve($abs);
    $rootLex = lexicalResolve(ROOT);
    return str_starts_with($real, $rootLex . '/') || $real === $rootLex ? $abs : null;
}
...
function serveStatic(string $path, string $contentType): void
{
    if (!is_file($path)) {
        http_response_code(404);
        header('Content-Type: text/plain; charset=utf-8');
